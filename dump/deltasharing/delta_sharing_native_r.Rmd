...
# Method 2: If you encounter C stack issues (common in containers)
# First install dependencies separately:
# install.packages(c("bit", "hms", "bit64", "assertthat", "progress"))
# Sys.setenv("LIBARROW_MINIMAL" = "false")
# install.packages("arrow")

# Then install the main package:
# options(expressions = 500000)
# Sys.setenv("R_C_STACK_LIMIT" = "2147483647")
# remotes::install_github("zacdav-db/delta-sharing-r")
```
...
if (requireNamespace("delta.sharing", quietly = TRUE)) {
  ...
  cat("✅ delta.sharing package loaded successfully\n")
  cat("Package version:", packageVersion("delta.sharing"), "\n")
} else {
  cat("❌ delta.sharing package not installed\n")
...
if (time_until_expiry < 24) {
  cat("⚠️  WARNING: Token expires within 24 hours!\n")
}
```
...
tryCatch({
  ...
  if (exists("delta_sharing_client")) {
    ...
    cat("✅ Client created with delta_sharing_client()\n")
  } else if (exists("DeltaSharingClient")) {
    client <- DeltaSharingClient(config_path)
    cat("✅ Client created with DeltaSharingClient()\n")
  } else if (exists("sharing_client")) {
    client <- sharing_client(config_path)
    cat("✅ Client created with sharing_client()\n")
  } else {
    cat("❓ Please check package documentation for client creation\n")
    cat("Available functions:\n")
    print(ls("package:delta.sharing"))
  }
}, error = function(e) {
  cat("❌ Error creating client:", e$message, "\n")
...
if (exists("client")) {
  tryCatch({
    ...
    if (exists("list_shares")) {
      ...
      cat("❓ Please check documentation for list_shares function\n")
      shares <- NULL
    }
    ...
    cat("❌ Error listing shares:", e$message, "\n")
  })
} else {
  cat("❌ No client available - please check client creation above\n")
}
```
...
if (exists("share_name")) {
  tryCatch({
    if (exists("list_schemas")) {
      ...
      cat("❓ Please check documentation for list_schemas function\n")
      schemas <- NULL
    }
    ...
    cat("❌ Error listing schemas:", e$message, "\n")
  })
}
```
...
if (exists("schema_name")) {
  tryCatch({
    if (exists("list_tables")) {
      ...
      cat("❓ Please check documentation for list_tables function\n")
      tables <- NULL
    }
...
if (exists("table_name")) {
  tryCatch({
    ...
    if (exists("load_table")) {
      ...
      cat("❓ Please check documentation for data loading function\n")
      df <- NULL
    }
...
if (requireNamespace("delta.sharing", quietly = TRUE)) {
  cat("✅ Native R delta.sharing package is available\n")
  cat("✅ Provides pure R implementation (no Python required)\n")
  cat("✅ Should integrate well with renv\n")
  
  if (exists("df") && !is.null(df)) {
    cat("✅ Successfully loaded data:", nrow(df), "rows\n")
  }
  ...
  cat("❌ Native R delta.sharing package not installed\n")
