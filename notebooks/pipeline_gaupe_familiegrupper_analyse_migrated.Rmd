...
dat_ordered <- order_observations(dat, time_column = "datotid_fra")
time_mat <- create_time_matrix(dat_ordered$datotid_fra, dat_ordered$datotid_til)
dist_mat <- create_distance_matrix(dat_ordered$geometry)
rule_mat <- apply_distance_rules(time_mat, dat_ordered$byttedyr)  # uses lut_distance_rules by default

# fam_groups <- <your grouping logic here>
fam_groups <- NULL  # Placeholder
```
